...
def get_weather():                                            # 날씨 정보 파싱용 함수
    API_KEY="xxxxx"                                 # openweathermap에서 발급받은 API키 입력
    API_ADDRESS="http://api.openweathermap.org/data/2.5/weather?q=Seoul&appid={}".format(API_KEY) #API주소
...
if samsung:                                         # 삼성 주식정보가 값이 들어 있으면
    ...
    draw.text((8, 35),samsung['up_down_price'], inky_display.BLACK, stock_font2) # draw 객체에 상승 및 하락 가격 표시
else:
    pirnt("Warning, no Stock information found!")           # 삼성 주식 정보가 없으면 경고 문구 터미널에 표시


if weather:                                           # 날씨 정보 값이 있으면
    draw.text((90,78), str(round(weather['temp'],1))+"C", inky_display.BLACK, weather_font2) # draw객체에 현재 온도 표시
...
if weather_icon is not None:                                      # 표시할 날씨 아이콘 변수에 날씨가 설정되어 있으면
    bg_img.paste(icons[weather_icon], (88, 33))                   # icons에서 표시할 날씨의 아이콘 이미지를 불러와서 bg_image에 그려줌.
else:                                                             # 표시할 날씨 아아콘 변수가 없으면
    draw.text((88, 33), "?", inky_display.BLACK, font=weather_font)   # 물음표 표시




dt = dt.datetime.now()                                           # datetime객체에서 현재 datetime정보를 불러옴
dt_msg = dt.strftime("%m-%d %H:%M")                            # 날짜 및 시간를 표시해줄 형식 지정
